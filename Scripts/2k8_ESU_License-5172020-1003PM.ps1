            ...
            If($?)
            {
              if($OSVersion -like "6.1*") #Windows Server 2008 R2
              {
                ...
                If($?)
                {
                  ...
                  Try{
                      $ClprtKBUnins = Invoke-command -cn $ServerName -ScriptBlock{
                        ...
                        return "Culprit KB sucessfully Uninstalled. Re-run ESU script for this server."
                      }
                  }Catch{
                ...
                Else{ #No Culprit KB, proceed to ESU process
                       ...
                       if($getHF.hotfixid.count -lt 4)
                       {
                           ...
                           if($chkUnzip){
                                ...
                                $remotelastexitcode = invoke-command -ScriptBlock { return $?} -Session $remotesession
                                 ...
                                 if($patchInstall -match "Need manual reboot"){
                                        $patchInstallNote = "Need manual reboot to complete patch installation."
                                 }
                  ...
                  $temp.Notes = $prechckLic | Select-String -pattern "License"
                  $temp.Error = $errorcount
                  $reports += $temp
                  }
                }
